...
const Achievement: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                await fetchAchievements(); // Refetch achievements
            } else {
    ...
    const handleDelete = async (achievementId: number) => {
        ...
        try {
            const response = await fetch("/api/deleteAchievements", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
                    ...
                    <div className="space-y-4">
                        {Array.isArray(achievements) && achievements.length > 0 ? (
                            achievements.map((achievement, index) => (
                                <div key={index} className="border p-4 rounded shadow-md flex justify-between items-center">
                                    <div>
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <FaTrashAlt className="cursor-pointer text-red-600" onClick={() => handleDelete(achievement.id)} />
                                    </div>
                                </div>
                            ))
                        ) : (
